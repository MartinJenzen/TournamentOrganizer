import { KnockoutRound, Prisma } from '@prisma/client';

export async function generateMatchFixtures(transaction: Prisma.TransactionClient, tournamentId: number, tournamentType: string, teams: any[], groups: any[] = [], matchesPerTeam: number, knockoutLegs: number = 0) {
  let matches: any[] = [];
...
async function generateFirstRoundOfCupFixtures(transaction: Prisma.TransactionClient, tournamentId: number, teams: any[], knockoutLegs: number) {
...
// TODO: refactor to break up into smaller functions
export async function generateInitialKnockoutMatchesFromGroups(transaction: Prisma.TransactionClient, tournament: any) {
  ...
  const initialKnockoutMatches: any[] = [];
  ...
  if (qualifiedTeamsPerGroupCount === 1 && groupsCount > 1) {
    for (let i = 0; i < groupsCount; i += 2) {
      ...
      const teamX = groupX.teams.find((team: any) => team.position === 1);
      const teamY = groupY.teams.find((team: any) => team.position === 1);      
  ...
  // If 2 or more teams advance from each group, pair best qualified team from group X vs. worst qualified team from group Y, etc.
    ...
    for (let i = 0; i < groupsCount; i += 2) {
      ...
      for (let j = 0; j < qualifiedTeamsPerGroupCount; j++) {
        const teamX = groupX.teams.find((team: any) => team.position === (j + 1));
        const teamY = groupY.teams.find((team: any) => team.position === (qualifiedTeamsPerGroupCount - j));
  ...
  // If only one group exists and multiple teams advance, pair best qualified team vs. worst qualified team, second best vs. second worst, etc.
    ...
    for (let i = 0; i < qualifiedTeamsCount / 2; i++) {
      ...
      const teamX = groups[0].teams.find((team: any) => team.position === (i + 1));
      const teamY = groups[0].teams.find((team: any) => team.position === (qualifiedTeamsCount - i));
